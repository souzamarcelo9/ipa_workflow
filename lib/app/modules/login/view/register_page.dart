...
import 'package:viska_erp_mobile/app/widgets/select_photo_options.dart';
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  signUp() async {
    ...
    if (isValid) {
      ...
      if (result.first == true) {
        ...
        if (result)
        {
          ...
          bool retorno =  await createProfessional(user);
          alertDialog(
              context,
              AlertType.error,
              AppLocalizations.of(context)!.attention,
              'Dados registrados com sucesso. Aguarde a aprovação de seu perfil pelo administrador do sistema.');
   ...
   Future<bool> createProfessional(UserModel usuario) async {
       bool criado = false;
       ...
       _profissionalModel.email = usuario.email;
       _profissionalModel.flgAdm = 'Usuario';
       _profissionalModel.telefone = usuario.phone;
       _profissionalModel.obs = 'Registrado pelo usuário através do celular';

       try {
         docRef = await db
             .collection(FirebaseConst.profissionais)
             .withConverter(
           fromFirestore: ProfissionalModel.fromFirestore,
           toFirestore: (value, options) {
             return _profissionalModel.toFirestore();
           },
         )
             .add(_profissionalModel);
         criado = true;
       }
       catch (e)
       {
